...
from utils import laplacian_filtering
...
def train_epoch(model: Model, optimizer, features, features_hat, adj, adj_hat, subgraphs, epoch, bmm_model):
    # train mode
    model.train()
    ...
    for batch_index in range(num_batches):
        ...
        node_embed_hat, q_hat = model(features_hat, adj_hat)
        ...
        subgraph_embed = torch.mean(bf, dim=1)
        subgraph_hat_embed = torch.mean(bf_hat, dim=1)
        ...
        N = node_embed_index.size(0)
        if epoch < args.epoch_start:
            ns_loss, nn_loss, ss_loss, _, _, _ = loss_cal(model, subgraph_embed, subgraph_hat_embed, node_embed_index, node_embed_hat_index)
        else:
            _, _, _, nn_sim, ns_sim, ss_sim = loss_cal(model, subgraph_embed, subgraph_hat_embed, node_embed_index, node_embed_hat_index)
            ...
            confidence = torch.nonzero(B < (1 - args.hard_ratio), as_tuple=False).flatten()
            confidence_hat = confidence + N
            confidence_combinations = torch.combinations(confidence, r=2)
            confidence_combinations_hat = torch.combinations(confidence_hat, r=2)
            combinations = torch.cat([confidence_combinations, confidence_combinations_hat], dim=0)
            nn_B = torch.ones((N * 2, N * 2)).cuda()
            ns_B = torch.ones((N * 2, N * 2)).cuda()
            ss_B = torch.ones((N * 2, N * 2)).cuda()
            nn_B[combinations[:, 0], combinations[:, 1]] = torch.abs(Q[combinations[:, 0], combinations[:, 1]]
                                                                     - nn_sim[combinations[:, 0], combinations[:, 1]]) ** args.tao
            nn_B[combinations[:, 1], combinations[:, 0]] = torch.abs(Q[combinations[:, 1], combinations[:, 0]]
                                                                     - nn_sim[combinations[:, 1], combinations[:, 0]]) ** args.tao
            ns_B[combinations[:, 0], combinations[:, 1]] = torch.abs(Q[combinations[:, 0], combinations[:, 1]]
                                                                     - ns_sim[combinations[:, 0], combinations[:, 1]]) ** args.tao
            ns_B[combinations[:, 1], combinations[:, 0]] = torch.abs(Q[combinations[:, 1], combinations[:, 0]]
                                                                     - ns_sim[combinations[:, 1], combinations[:, 0]]) ** args.tao
            ss_B[combinations[:, 0], combinations[:, 1]] = torch.abs(Q[combinations[:, 0], combinations[:, 1]]
                                                                     - ss_sim[combinations[:, 0], combinations[:, 1]]) ** args.tao
            ss_B[combinations[:, 1], combinations[:, 0]] = torch.abs(Q[combinations[:, 1], combinations[:, 0]]
                                                                     - ss_sim[combinations[:, 1], combinations[:, 0]]) ** args.tao

            ns_loss, nn_loss, ss_loss, _, _, _ = loss_cal(model, subgraph_embed, subgraph_hat_embed, node_embed_index,
                                                          node_embed_hat_index, nn_B, ns_B, ss_B)

        Z = node_embed_index
        Zs.append(Z)
        # make cluster loss to the same order of magnitude
        loss_clu = F.kl_div(q.log(), p, reduction='batchmean')
        loss = args.beta * ns_loss + (1 - args.beta) * nn_loss + args.alpha * ss_loss + args.gamma * loss_clu
    ...
    with torch.no_grad():
        ...
        return Z, loss, output_q, model.cluster_layer


def loss_cal(model, subgraph_embed, subgraph_hat_embed, node_embed_index, node_embed_hat_index, nn_b=1, ns_b=1, ss_b=1):
    ...
    pos_neg = mask.cuda() * torch.exp(ns_sim * ns_b)
    ...
    pos_neg = mask.cuda() * torch.exp(nn_sim * nn_b)
